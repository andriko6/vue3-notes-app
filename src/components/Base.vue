  ...
  <header>
    ...
    <slot name="filter">
      <div class="action-filter">
        <label for="preset">Filter by Category:</label>
        <select id="preset" class="selector" v-model="selectedPreset">
...
export default {
  ...
  computed: {
    ...
    filteredNotes() {
     if (this.selectedPreset) {
       return this.notes.filter(e => e.preset === this.selectedPreset);
     } else {
       return this.notes;
     }
    },
  ...
  mounted() {
    ...
    this.$watch (() => [state.noteTitle, state.noteText], (value) => {
